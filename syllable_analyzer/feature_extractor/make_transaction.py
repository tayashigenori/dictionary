...
OUTPUT_PATH_BASE = os.path.dirname(os.path.abspath(__file__)) + "/transaction/%d.csv"
        ...
        try:
            ...
            for l in sf.readlines():
                ...
                try:
                    ...
                    if mand != '': line_r.append( Hanzi( split(mand), is_tone_numeral = False) )
                    if cant != '': line_r.append( Honzi( split(cant) )                         )
                    if ko   != '': line_r.append( Hanja( split(ko_roman) )                     )
                    if viet != '': line_r.append( Hantu( split(viet) )                         )
                    if ja   != '': line_r.append( Kanji( split(ja) )                           )
                    transaction_list = list( map( get_transaction_from_hanzi, line_r) )
                    f_result.append( ",".join( [char] + transaction_list )  )
                #except TypeError:
