...
OUTPUT_PATH_BASE = os.path.dirname(os.path.abspath(__file__)) + "/vector/%d.csv"
...
def get_vector_from_hanzi(hz):
    if hz == '':
        return ''
    l = list( map (lambda x: str(x), hz.vectorize()) )
    return ",".join( l )
        ...
        try:
            ...
            for l in sf.readlines():
                ...
                try:
                    ...
                    vector_list = list( map( get_vector_from_hanzi, line_r) )
                    f_result.append( "\t".join( [char] + vector_list )  )
                #except TypeError:
